...
namespace til // Terminal Implementation Library. Also: "Today I Learned"
{
    ...
    public:
        ...
        void fill(const value_type& value, const size_type start = 0)
        {
            insert(value, start, gsl::narrow_cast<size_type>(_size - start));
        }

        // Inserts this value at the given position for the given run length.
        void insert(const value_type value, const size_type position, const size_type length = 1)
        {
            // TODO: validate position in bounds?
            std::pair<T, S> item{ value, length };
            _merge(&item, &item + 1, position);
        }

        // Inserts all values between first and last starting at the given position.
        template<typename InputIterator>
        void assign(InputIterator first, InputIterator last, const size_type position = 0)
        {
